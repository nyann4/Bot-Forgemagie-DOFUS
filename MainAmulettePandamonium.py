from screen import *
from CalculAmulettePandamonium import *
...
def MainAmulettePandamonium(poid):
    carac_position = [0.2, 1.0, 3.0, 100.0, 51.0, 5.0, 5.0, 10, 0.1, 6.0, 6.0, 7] #liste de Pui par unité par ligne
    X, done, type_rune, compteur_erreur=  0, False, 0, 0 # affection des valeurs a chaque lancement du programme
    error, highest_rune, second_highest = True, 100, 51
    tenta, tenta_plus = 0 , 0
    nombre_ligne_item = 12
    max_rune1, max_rune2, max_rune3 = 401, 71, 501

    while(done != True): #Boucle principale
        pui = []
        time.sleep(0.52) #timer minimal pour qu'aucune erreur de pui ne soit produite
        ...
        if X != 0 :
            ...
            if ((old_stat[3] - pui[3]) == 100) or ((old_stat[4] - pui[4]) == 51) or poid > 5.9:
        ...
        time.sleep(0.10)
        type_rune, indicatif, done, tenta= UpStats2(list_value, poid, nombre_item, nombre_ligne_item, tenta, max_rune1, max_rune2, max_rune3)
        print("resistance feu % " ,pui[8])
        if (type_rune == 100 ):
            list_1 = screen(750, (340+40*nombre_ligne_item), 200, 40, 2)
            if list_1[0] == '3% Résistance Terre' :
                tenta_plus += 1
                list_value = screenValues(760, 341, 200, 40*(nombre_ligne_item+1), 1)
...
with Listener(on_press=lambda k: None, on_release=on_release) as listener:
    MainAmulettePandamonium(poid) #lancement du main dans un listener pour pouvoir le stoper a la pression d'une touche a tout moment
    listener.stop()