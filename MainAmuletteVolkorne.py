from screen import *
from CalculAmuletteVolkorne import *
...
def MainAmuletteVolkorne(poid, nombre_ligne_item, mode, double_exo, useless_stat, under1_root_path, path_dict_item):
    carac_position = [0.2, 1.0, 3.0, 10.0, 100.0, 30.0, 5.0, 3.0, 6.0, 7.0, 5.0]    #liste de Pui par unité par ligne
    X, done, type_rune, other_rune=  0, False, 0, False                                               #affection des valeurs a chaque lancement du programme
    highest_rune, second_highest = 100, 30
    tenta, other = 0, False
    ...
    while(done != True): #Boucle principale
        ...
        if X == 0:
            ...
            with open(path_pui_item, "w", encoding= "utf-8") as outfile:            #ecris le pui lu sur l'item
                json.dump(data_pui, outfile)
        if X != 0 :
            with open(path_pui_item, encoding='utf-8') as outfile:
                data_pui =json.load(outfile)
            old_stat = data_pui['pui']                                              #recupère le pui lu a la rune précédente
            diff = float((sum(pui) - sum(old_stat)))                                #fait la différence de pui de l'item
            diff_rune_indic = pui[indicatif] - old_stat[indicatif]                  #fait la différence de pui sur la ligne de la rune passé
            reliquat_reading, lag = reliquat_true(X, path_dict_item)                #verifie si le mot reliquat apparait ou si l'historique est identique au précédent
            if lag or old_stat == pui:
                list_value, pui = get_stats(nombre_ligne_item, carac_position)      #réitère la boucle si l'historique était identique ou que l'item n'a pas changé
            ...
            data_pui['pui'] = pui
            with open(path_pui_item, "w", encoding= "utf-8") as outfile:            #ecris le pui actuel
                json.dump(data_pui, outfile)
...
print("entrez le mode : 1 pour RePerFeu // 2 pour OverVita // 3 pour DoSort")
exo = int(input())
print("entrez 1 pour double exo 0 pour exo normaux")
double_exo =int(input())
nom_item = "Amulette Volkorne"

# definit la reliquat a lire commme "aaaa" pour qu'il ne soit pas similaire au dernier lu
...
with open(path_dict_item, "w", encoding= "utf-8") as outfile: #pui_item
    json.dump(data, outfile, ensure_ascii=False)

# definit les valeurs  par défaut  de la dernière rune lu et de sa position
path_pui_item = under1_root_path / "pui_item/pui.json"
with open(path_pui_item, encoding='utf-8') as outfile:
...
with Listener(on_press=lambda k: None, on_release=on_release) as listener:
    MainAmuletteVolkorne(poid, nombre_ligne_item, mode, double_exo, useless_stat, under1_root_path, path_dict_item) #lancement du main dans un listener pour pouvoir le stoper a la pression d'une touche a tout moment
    listener.stop()