from screen import *
from CalculSoleretsDeGuerre import *
...
def MainSoleretsDeGuerre(poid):
    carac_position = [0.2, 1.0, 3.0, 10.0, 90.0, 5.0, 6.0, 6.0, 4.0, 4.0, 7.0, 5.0] #liste de Pui par unité par ligne
    X, done, type_rune, compteur_erreur=  0, False, 0, 0 # affection des valeurs a chaque lancement du programme
    error, highest_rune, second_highest = True, 90, 10000
    ...
    while(done != True): #Boucle principale
        ...
        if X != 0 :
            ...
            if ((old_stat[4] - pui[4]) == 90) or ((old_stat[5] - pui[5]) == 3000) or poid > 2.9:
        ...
        time.sleep(0.10)
        type_rune, indicatif, done= UpStats(list_value, poid, nombre_item,pui, nombre_ligne_item)
        print("resistance feu % " ,pui[8])
        if (type_rune == 100 and pui[8] > 77):
            tenta += 1
            suivis_value(list_value, tenta, pui, nom_item)
        if  type_rune ==51 :
            tenta_plus += 1
            suivis_value(list_value, tenta, pui, nom_item)
        X += 1
        ...
        if X%(count*1500) == 0 : 
            time.sleep(0.5)
            Reset_stand(nom_item, 1587, 91, 'cordomage') #ferme et réouvre l'atelier
            FirstItem()
...
with Listener(on_press=lambda k: None, on_release=on_release) as listener:
    MainSoleretsDeGuerre(poid) #lancement du main dans un listener pour pouvoir le stoper a la pression d'une touche a tout moment
    listener.stop()