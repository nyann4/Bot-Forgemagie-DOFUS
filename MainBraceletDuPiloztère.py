from screen import *
from CalculBraceletDuPiloztère import *
...
def MainBraceletPiloztère(poid, nombre_ligne_item, mode, useless_stat, under1_root_path, path_dict_item):
    carac_position = [0.2, 2.0, 10.0, 51.0, 5.0, 5.0, 5.0, 6.0] #liste de Pui par unité par ligne
    ...
    while(done != True): #Boucle principale
        ...
        type_rune, indicatif, done, tenta, poid, other, tenta_po = UpStats(list_value, poid, nombre_item, nombre_ligne_item, tenta, nom_item, mode, other, pui, tenta_po)
        if X > 8000:
            return True
        X += 1
        print('### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ###')
...
print("entrez le mode : 1 pour Docrit // 2 pour OverVita ")
exo = int(input())
nom_item = "Bracelet du Piloztère"
...
with Listener(on_press=lambda k: None, on_release=on_release) as listener:
    MainBraceletPiloztère(poid, nombre_ligne_item, mode, useless_stat, under1_root_path, path_dict_item) #lancement du main dans un listener pour pouvoir le stoper a la pression d'une touche a tout moment
    listener.stop()