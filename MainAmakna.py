...
from screen import *
from CalculAmakna import UpStats
...
def MainAmakna():
    ...
    while(done != True):
        ...
        done, tenta, tenta_plus= UpStats(list_value, nombre_item, tenta, tenta_plus, nombre_ligne_item, type_exo)
        X += 1
        count = 1
        ...
        if X%(count*15000) == 0 : 
            time.sleep(0.5)
            Reset_stand(nom_item, 1587, 91, 'costumage') #ferme et réouvre l'atelier !!!!! a modifier
            FirstItem()
